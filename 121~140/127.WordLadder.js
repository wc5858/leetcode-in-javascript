...
//   let cache = {}
//   function helper(beginWord, endWord, deep) {
//     if (cache[endWord + '-' + deep]) return cache[endWord + '-' + deep]
//     if (checkDiffer(beginWord, endWord)) return deep + 1
//     let min = Infinity
//     let data = [...set.values()]
//     for (let i of data) {
//       if (checkDiffer(i, endWord)) {
//         set.delete(i)
//         min = Math.min(helper(beginWord, i, deep + 1), min)
//         set.add(i)
//       }
//     }
//     cache[endWord + '-' + deep] = min
//     return min
//   }
//   if (set.has(endWord)) {
//     set.delete(endWord)
//     return helper(beginWord, endWord, 1) | 0
//   }
//   return 0
// };
// 参考了discuss里的方案进行优化
let ladderLength = function (beginWord, endWord, wordList) {
  ...
  let cur = [beginWord], next
  let ans = 1
  while (cur.length > 0) {
    next = []
    for (let i = 0; i < cur.length; i++) {
      if (cur[i] === endWord) return ans
      next = next.concat(getNext(cur[i]))
    }
    cur = next
    ans++
  }
...
let testCases = [
  {
    data: ["hit", "cog", ["hot", "dot", "dog", "lot", "log", "cog"]],
    res: 5,
  },
  {
    data: ["hit", "cog", ["hot", "dot", "dog", "lot", "log"]],
    res: 0,
  },
  {
    data: ["hot", "dog",
